...
if __name__== "__main__":
    ...
    xfile = "Path to a txt file that contains embeddings"
    yfile = "Path to a txt file that contains labels"
    
    X = torch.tensor(np.loadtxt(xfile), dtype=torch.float32)
    labels = torch.tensor(np.loadtxt(yfile), dtype=torch.long)
